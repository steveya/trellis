...
def test_derivative_method_taxonomy_covers_runtime_and_matrix_methods():
    ...
    hybrid_hvp_payload = derivative_method_payload("hybrid_scalar_vector_hvp")
    assert hybrid_hvp_payload["derivative_method_category"] == "hybrid_ad"
    assert hybrid_hvp_payload["derivative_method_support"] == "partial"
    assert hybrid_hvp_payload["backend_operator"] == "hessian_vector_product"

    hybrid_matrix_payload = derivative_method_payload("hybrid_matrix_vector_vjp")
    assert hybrid_matrix_payload["derivative_method_category"] == "hybrid_ad"
    assert hybrid_matrix_payload["derivative_method_support"] == "partial"
    assert hybrid_matrix_payload["backend_operator"] == "vjp"

    hybrid_matrix_hvp_payload = derivative_method_payload("hybrid_matrix_vector_hvp")
    assert hybrid_matrix_hvp_payload["derivative_method_category"] == "hybrid_ad"
    assert hybrid_matrix_hvp_payload["derivative_method_support"] == "partial"
    assert hybrid_matrix_hvp_payload["backend_operator"] == "hessian_vector_product"

    unsupported_jvp_payload = derivative_method_payload(
        "unsupported_hybrid_jvp",
        requested_backend_operator="jvp",
    )
    assert unsupported_jvp_payload["derivative_method_category"] == "unsupported"
    assert unsupported_jvp_payload["derivative_method_support"] == "unsupported"
    assert unsupported_jvp_payload["requested_backend_operator"] == "jvp"
    assert "backend_operator" not in unsupported_jvp_payload
