...
| L46 | **Equity-vol surface authority is now repaired-surface aware, but still bounded to per-expiry raw-SVI plus discrete calendar repair** — Trellis can now fit a multi-expiry repaired equity-vol authority, derive local vol from that repaired surface, and compare that surface stage against a later Heston fit, but the checked path is still not a full production quote-cleaning, SSVI, stochastic-local-vol, or SPX/VIX-style volatility program | Trellis now has a real market-object-first equity-vol slice, but it is still not a full industrial equity-vol plant | `trellis/models/calibration/equity_vol_surface.py`, `trellis/models/calibration/local_vol.py`, `trellis/models/calibration/heston_fit.py`, `docs/mathematical/calibration.rst` |
