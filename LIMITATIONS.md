...
| L43 | **Static-leg closure is still bounded and only partly executable** — route-free decomposition and first lowering admission now exist for vanilla fixed-float IRS, SOFR/FF-style basis swaps, and fixed coupon bonds, and the swap/bond families can materialize checked repository engines directly; but basis swaps still have no checked executable lowering and none of the static-leg families are yet on the authoritative fresh-build compiler path | Trellis can now express and structurally admit the first static-leg cohort without route ids, but it is still not support-contract-correct to claim full route-free execution for generic leg products or basis swaps | `trellis/agent/static_leg_contract.py`, `trellis/agent/static_leg_admission.py`, `trellis/agent/knowledge/decompose.py`, `docs/quant/static_leg_contract_ir.rst` |
| L44 | **Dynamic event/state/control closure is admission-only today** — `DynamicContractIR` now carries explicit inventory and control-magnitude semantics, bounded decompositions exist for autocallable/TARN, callable-bond/swing, and GMWB-style proving fixtures, `compile_dynamic_lane_admission(...)` emits admitted automatic, discrete, and continuous lane contracts with benchmark plans, and insurance-style overlay terms now fail closed at the bounded decomposition/admission boundary; but none of those lanes is yet wired into the authoritative fresh-build pricing compiler | Trellis can now classify and structurally admit the first dynamic cohorts without route-local product authority, and it now blocks mortality/lapse/fee-bearing control relatives instead of silently flattening them into the financial-control core; but it is still not support-contract-correct to claim route-free executable pricing for autocallables, TARN/TARF, callable coupon structures, swing options, or GMxB-style control families until a checked numerical compiler path lands | `trellis/agent/dynamic_contract_ir.py`, `trellis/agent/dynamic_lane_admission.py`, `trellis/agent/semantic_track_classifier.py`, `trellis/agent/knowledge/decompose.py`, `docs/quant/dynamic_contract_ir.rst` |
