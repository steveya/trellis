...
| L43 | **Static-leg closure is still bounded and only partly executable** — route-free decomposition and first lowering admission now exist for vanilla fixed-float IRS, SOFR/FF-style basis swaps, fixed coupon bonds, and scheduled `period_rate_option_strip` contracts; the swap, bond, and scheduled strip families can now materialize checked repository engines directly, including analytical and Monte Carlo cap/floor helpers for the strip family, but basis swaps still have no checked executable lowering and none of the static-leg families are yet on the authoritative fresh-build compiler path | Trellis can now express, decompose, and materially lower a broader bounded static-leg surface without route ids, including the canonical leg-semantic home for scheduled cap/floor strips, but it is still not support-contract-correct to claim full route-free execution for generic leg products, basis swaps, or authoritative fresh-build cutover for the static-leg cohort | `trellis/agent/static_leg_contract.py`, `trellis/agent/static_leg_admission.py`, `trellis/agent/knowledge/decompose.py`, `docs/quant/static_leg_contract_ir.rst` |
