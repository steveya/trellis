...
| L44 | **Dynamic event/state/control closure is representational only in the first slice** — `DynamicContractIR`, explicit event ordering, state schema, controller/action semantics, classifier coverage, and a bounded callable-bond decomposition now exist, but there is still no admitted automatic-state, discrete-control, or continuous-control lowering lane | Trellis can now represent dynamic contract structure route-independently, but it is still not support-contract-correct to claim executable pricing support for callable coupon products, autocallables, TARN/TARF, swing options, or GMxB-style control families | `trellis/agent/dynamic_contract_ir.py`, `trellis/agent/semantic_track_classifier.py`, `trellis/agent/knowledge/decompose.py`, `docs/quant/dynamic_contract_ir.rst` |
