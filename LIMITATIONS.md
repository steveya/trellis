...
| L43 | **Static-leg closure is still bounded and not yet on the authoritative fresh-build path** — route-free decomposition and checked lowering admission now exist for vanilla fixed-float IRS, SOFR/FF-style basis swaps, fixed coupon bonds, and scheduled `period_rate_option_strip` contracts; those bounded families can now materialize checked repository engines directly, including analytical and Monte Carlo cap/floor helpers for the strip family and an explicit floating-vs-floating basis-swap helper, but the static-leg cohort still is not the authoritative fresh-build compiler path and still excludes richer leg structures outside the admitted constant-notional term/overnight strip and basis families | Trellis can now express, decompose, and materially lower a broader bounded static-leg surface without route ids, including the canonical leg-semantic home for scheduled cap/floor strips and the first executable basis-swap lane, but it is still not support-contract-correct to claim full route-free execution for generic leg products or full fresh-build cutover for the static-leg cohort | `trellis/agent/static_leg_contract.py`, `trellis/agent/static_leg_admission.py`, `trellis/agent/knowledge/decompose.py`, `trellis/models/rate_basis_swap.py`, `docs/quant/static_leg_contract_ir.rst` |
