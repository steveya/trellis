...
| # | Limitation | Impact | Files |
|---|-----------|--------|-------|
| L41 | **ContractIR solver compilation is still shadow-only and intentionally excludes arithmetic Asians** — the checked structural compiler can now bind vanilla, digital, swaption, two-asset basket/spread, and equity variance families into bounded solver calls and emit comparison provenance, but runtime route authority is still unchanged and arithmetic Asians still fail closed on the structural path pending a real checked helper surface | Fresh builds are not yet route-free in production; any claim that Phase 4 route retirement or arithmetic-Asian structural migration is already complete would be incorrect | `trellis/agent/contract_ir_solver_compiler.py`, `trellis/agent/semantic_contract_compiler.py`, `docs/quant/contract_ir.rst`, `docs/developer/contract_ir_solver_compiler.rst` |
