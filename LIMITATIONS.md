...
| L26 | **Curve bootstrap is still not fully desk-complete** — Trellis now supports a first dated, dependency-aware multi-curve bootstrap lane, but the broader curve plant still uses raw NumPy least-squares solves, has no smoothing or regularized family selection, and only supports a bounded subset of schedule and futures conventions | Curve construction is materially stronger than the old tenor-only bootstrap, but it is still too narrow for production multi-curve calibration or stable adjoint sensitivities | `trellis/curves/bootstrap.py`, `trellis/data/resolver.py`, `docs/mathematical/calibration.rst` |
...
| L29 | **Advanced stochastic-vol integration is still partial** — supported Heston smile calibration, repaired-surface-driven full-surface Heston compression, and runtime binding now exist, but the checked path still fits one global Heston parameter pack across the surface and broader cross-engine stochastic-vol reuse remains incomplete | Trellis can now compress a bounded repaired equity-vol authority into one reusable Heston parameter set, but not a full time-dependent stochastic-vol surface plant, stochastic-local-vol bridge, or uniform reuse across all stochastic-vol families | `trellis/models/calibration/heston_fit.py`, `trellis/models/processes/heston.py`, `trellis/models/processes/sabr.py`, `docs/mathematical/calibration.rst` |
| L45 | **Single-name credit calibration is now CDS-pricer-backed, but still bounded to a canonical running-spread workflow** — spread and hazard quotes now normalize through the analytical CDS pricer and the solve objective fits par running spreads with explicit repricing / survival / forward-hazard diagnostics, but the checked path still assumes one quarterly `ACT/360` tenor schedule from `market_state.settlement` and does not yet cover standard-coupon-plus-upfront, IMM-roll, holiday-calendar, or index-credit governance | Trellis can now honestly calibrate bounded running-CDS strips through a pricing engine, but it is still not a full production CDS bootstrap or broader credit calibration stack | `trellis/models/calibration/credit.py`, `trellis/models/credit_default_swap.py`, `docs/mathematical/calibration.rst` |
