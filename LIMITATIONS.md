...
| L43 | **Static-leg closure is still bounded and only partly executable** — route-free decomposition and first lowering admission now exist for vanilla fixed-float IRS, SOFR/FF-style basis swaps, and fixed coupon bonds, and the swap/bond families can materialize checked repository engines directly; the static-leg ADT now also has a first-class scheduled `period_rate_option_strip` representation, but that family still has no route-free decomposition or executable lowering, basis swaps still have no checked executable lowering, and none of the static-leg families are yet on the authoritative fresh-build compiler path | Trellis can now express a broader bounded static-leg surface without route ids, including the canonical leg-semantic home for scheduled cap/floor strips, but it is still not support-contract-correct to claim full route-free execution for generic leg products, basis swaps, or scheduled rate-option strips | `trellis/agent/static_leg_contract.py`, `trellis/agent/static_leg_admission.py`, `trellis/agent/knowledge/decompose.py`, `docs/quant/static_leg_contract_ir.rst` |
