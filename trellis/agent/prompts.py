    ...
    *,
    ...
    if instrument_type in {"cds", "credit_default_swap"}:
        ...
        lines.extend([
            ...
            "- Keep module and class definition time declarative: docstrings, approved imports, frozen spec declarations, and the plain payoff declaration only; executable module/class control flow fails before import. The payoff constructor must store the submitted `spec` exactly as `self._spec`, and its sole `@property spec` must return that same object without alternate writers or dynamic attribute hooks.",
