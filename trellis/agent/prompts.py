    ...
    *,
    ...
    if instrument_type in {"cds", "credit_default_swap"}:
        ...
        lines.extend([
            ...
            "- Keep the complete CDS composition in one reachable `evaluate()` body with one direct final signed return. The exact fail-fast `ValueError` guards for missing `market_state.credit_curve` and `market_state.discount` are admitted; other conditional return/raise exits, unused or nested helpers, branch-hidden assembly, and statements after the final return do not satisfy the route contract.",
