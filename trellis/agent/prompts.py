    ...
    *,
    ...
    if instrument_type in {"cds", "credit_default_swap"}:
        ...
        lines.extend([
            ...
            "- Keep both module and `evaluate` imports on the exact CDS scaffold surface: direct, unaliased names from `__future__`, `dataclasses`, `datetime`, `trellis.conventions.calendar`, `trellis.conventions.schedule`, `trellis.core.date_utils`, `trellis.core.market_state`, `trellis.core.payoff`, `trellis.core.types`, and `trellis.models.contingent_cashflows` only. Import `build_period_schedule`, the declared calendar/schedule convention symbols, and every selected default-event, first-event-weight, cashflow-constructor, and cashflow-PV primitive directly and without aliases from its public Trellis module. Module-style, relative, aliased, wildcard, and other arbitrary imports are never admitted. Do not shadow or rebind those names locally or at module scope.",
            "- Keep module and class definition time declarative: docstrings, approved imports, frozen spec declarations, and the plain payoff declaration only; executable module/class control flow fails before import. Prefer the leading `from __future__ import annotations` scaffold import. Without postponed annotations, every eager annotation name must already resolve to an unshadowed inert builtin or an approved direct type import; undefined, late, or rebound names fail closed before import. Every eager function default must likewise resolve to a literal, an unshadowed inert builtin, or an approved direct import available before the definition; enum attributes must name an existing member on that authoritative import, so undefined names, late or rebound enum bases, and nonexistent members fail closed. Literal dictionary keys and set elements must remain recursively hashable, so unconstructable nested container defaults fail before import. Eager annotation unions admit only proven type operands or `None`; arbitrary values and attribute/member expressions fail closed. Eager subscript annotations may use only the inert builtin `dict`, `frozenset`, `list`, `set`, `tuple`, and `type` bases; custom subscript bases fail closed before import. For spec declarations, preserve the directly imported `dataclasses.dataclass` binding and admit exactly one explicit `@dataclass(frozen=True)` class with the planned field order, annotations, required fields, and optional defaults; duplicate spec classes, changed defaults, extra fields, methods, and behavior hooks fail closed. Preserve the unshadowed builtin `property`, `staticmethod`, and `classmethod` decorator bindings at module and class scope. The payoff class may define only `__init__`, `spec`, `requirements`, and `evaluate`; extra helpers, lifecycle hooks, and data-model methods fail closed. The constructor must store the submitted `spec` exactly as `self._spec`, and its sole `@property spec` must return that same object without alternate writers or dynamic attribute hooks. Keep the exact inert `requirements` property returning only the literal `{'credit_curve', 'discount_curve'}` set.",
            "- Require every eager annotation union operation to contain at least one proven type operand. `None` may accompany a type-producing operand, but `None | None` and nested all-`None` union nodes fail closed before definition.",
            "- Frozen dataclass fields may not use mutable list, dictionary, or set literal defaults; use an immutable planned default instead.",
            "- Bind every frozen dataclass decorator to one earlier direct `from dataclasses import dataclass` import, and place every required field before any defaulted field.",
            "- Define the authoritative CDS method with exactly `evaluate(self, market_state)`: no positional-only, variadic, keyword-only, extra, or defaulted parameters are admitted.",
            "- Preserve the authoritative `self` receiver unchanged throughout `evaluate()`; never rebind or delete it before resolving the submitted `self._spec` economics.",
            "- Do not bind `dataclass`, `property`, `staticmethod`, or `classmethod` anywhere in `evaluate()`; local definitions must retain the authoritative imported or builtin decorator bindings.",
            "- Keep the payoff class body to optional docstrings, trusted `__trellis_comparison_bindings__` metadata, and the four canonical methods only; annotated fields and other class-level behavior declarations fail closed.",
            "- Keep the complete CDS composition in one reachable `evaluate()` body with one direct final signed return. Preserve the caller's `market_state` parameter unchanged; never rebind or delete it before using its active discount and credit curves. The exact fail-fast `ValueError` guards for missing `market_state.credit_curve` and `market_state.discount` are admitted only with the unshadowed builtin `ValueError` with one literal string message and no explicit cause; input-dependent assertions and other conditional return/raise exits, unused or nested helpers, branch-hidden assembly, and statements after the final return do not satisfy the route contract. Local definitions never count as composition evidence; before pruning their runtime bodies, local class bodies and local function/lambda defaults, decorators, and annotations must be definition-time declarative. Every assignment must be a simple binding consumed by the canonical composition before rebinding; dead, opaque, or non-composition writes fail closed. The canonical period loop and its mapped interval loop are the only admitted loops; additional `for`, `async for`, or `while` loops fail closed before smoke execution. Beyond the exact market guards, basis-point spread normalization, initial-survival empty-grid fallback, empty-period guard, and optional nonpositive-event guard, other `if`/ternary, `try`, `with`, `match`, comprehension, or standalone executable-expression constructs also fail closed.",
            ...
            "- For the bounded standard-CDS route, call `build_period_schedule` with exactly the three positional contract fields and the seven named `day_count`, `time_origin`, `calendar`, `bda`, `roll_convention`, `stub`, and `payment_lag_days` arguments; opaque `*args`/`**kwargs`, duplicate names, and extra keywords fail closed. Pass `calendar=WEEKEND_ONLY`, `bda=BusinessDayAdjustment.FOLLOWING`, `roll_convention=RollConvention.NONE`, `stub=StubType.SHORT_LAST`, and `payment_lag_days=0`; the current typed route does not admit convention overrides.",
            ...
            "- Compute survival to the first live interval with `credit_curve.survival_probability(event_grid.intervals[0].start_time)` (or `1.0` for an empty grid) and pass it as `initial_survival_weight`; forward-start weights must be unconditional from valuation. Call only the method-selected first-event primitive, with exactly one positional conditional-probability argument and only its declared explicit keywords, without `*args`, `**kwargs`, duplicate keywords, or extra controls; do not call the unselected primitive even for unused or diagnostic work.",
            ...
            "- Keep the authoritative `evaluate()` method undecorated on a plain payoff class, and never replace, rebind, or dynamically intercept that method after definition; CDS contract findings are blocking.",
            ...
            "- Keep the interval loop as a reachable direct child of the period loop. Preserve the unshadowed builtin bindings for `enumerate`, `float`, `getattr`, and `range`. Use `enumerate(event_grid.periods)` with no start argument and `range(interval_start, interval_stop)` with no keywords; never reassign the period index, period value, or interval index. Initialize `interval_start` exactly once with a reachable direct `0` assignment before the period loop, then mutate it only through the required mapped guard/tail updates. Initialize each premium, protection, event-accrual, and valuation-accrual accumulator exactly once to `0.0`, update it only with additive `+=` in its recognized loop, and do not write to it anywhere else before return. Each cashflow accumulator right-hand side must be exactly its corresponding directly imported public `coupon_cashflow_pv(...)` or `protection_payment_pv(...)` call; do not use attribute dispatch, aliases, direct or dynamic namespace shadowing, reflection, negation, scaling, or other wrappers. Bind `weight`, `default_probability`, and `discount_factor` on the direct cashflow constructor passed to that PV primitive; nested calls or decoy keywords do not satisfy the contract. Use each cashflow constructor's exact declared keyword surface: `CouponAccrual` requires only `notional`, `rate`, `accrual`, `discount_factor`, and `weight`, while `ProtectionPayment` requires only `notional`, `recovery`, `default_probability`, and `discount_factor`; either may add only the optional positive `sign`. Positional arguments, `**kwargs`, duplicates, missing fields, and extra keywords fail closed. Every simple value alias used as semantic evidence---including active spec, market curve, schedule, grid, weight, and cashflow-value aliases---must have one immutable assignment that dominates its use; reassigning an alias invalidates the evidence even if an earlier value was correct. Before scheduled-leg assembly, always apply the required period guard `if interval_stop <= interval_start: interval_start = interval_stop; continue`; the interval guard `if event_weight <= 0.0: continue` is also admitted before event-leg assembly. Other conditional `break` or `continue` exits fail closed. Do not reassign `interval_stop`, `interval_start`, or `event_weight` before those guards.",
            "- CDS running spreads may arrive as basis points. Normalize once at the top of `evaluate()` with `spread = float(spec.spread)` and `if spread > 1.0: spread *= 1e-4`, before the period loop and every spread alias or cashflow use; then use only the local decimal spread.",
