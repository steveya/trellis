...
_ALLOWED_TRADE_CHILDREN = {"swap", "swaption", "tradeHeader"}
...
def _normalize_inspected_fpml_document(
    ...
    *,
    ...
    if product_names not in {("swap",), ("swaption",)}:
    ...
    try:
        if product_names == ("swap",):
            ...
            if valuation_date is not None and _has_seasoned_floating_coupon(
                contract,
                valuation_date,
            ):
                _fail(
            ...
            if swaption is None:
                raise AssertionError("inspected swaption product is missing its element")
    ...
    *,
    ...
    contract = ContractIR(
        ...
        underlying=Underlying(
            ForwardRate(underlier_id, "lognormal_forward")
        ),
    ...
    premium_metadata = _normalize_swaption_premiums(
        swaption,
        namespace=namespace,
        valuation_date=valuation_date,
        known_party_ids=known_party_ids,
...
def _normalize_swaption_premiums(
    swaption,
    *,
    namespace: str | None,
    valuation_date: date | None,
    ...
    result = []
    for premium in _direct_children(swaption, "premium", namespace=namespace):
        _reject_unadmitted_direct_children(
            premium,
            allowed=_ALLOWED_PREMIUM_CHILDREN,
            scope="swaption_premium",
            namespace=namespace,
        )
        ...
        if (
            ...
            _fail(
                "contract_conflict:fpml_swaption_premium_parties",
                "contract_conflict",
                "Swaption premium parties must be distinct identified trade parties.",
            )
